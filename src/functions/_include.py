...
def identify_import_type(combined_line: str, ast_list: AST_LIST) -> dict:
    regex_patterns = {
    ...
    for regex in regex_patterns.values():
        ...
        if match:
            ...
            if not match.get('type', ""):
                match["type"] = os_path.splitext(match["path"])[1][1:].upper()
...
def include(ast_list: AST_LIST, current_scope, parent_scope, root_scope) -> str:
