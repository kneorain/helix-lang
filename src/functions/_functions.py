...
def extract_variables(ast_line: AST_LIST, root_scope) -> str:
    variables = {}
...
def process_modifiers(ast_list: AST_LIST, root_scope) -> list[str]:
...
def contains(line: AST_LIST, compare: tuple):
    # check if any of the tokens in compare are in line and
    return any([_ in line for _ in compare])

def function(ast_list: AST_LIST, current_scope, parent_scope, root_scope) -> str:
