...
from src.globals import NAMESPACED_KEYWORD, KEYWORDS
from src.globals import POOL

class Scope:
    name: str | AST_LIST
    namespace_header: AST_LIST
    namespace_type: str
    separate_file_namespace: bool
    children: list[AST_LIST]
    indent_level: int
    ...
    def append_line(self, line: AST_LIST) -> None:
        self.children.append(line)
        
    def append_child(self, line: AST_LIST) -> None:
        self.children.append(line)
    
    # AST_LIST contains:
    #   ast_list.indent_level: int
    #   ast_list.line: list[AST]
    #   ast_list.file: str
    #
    # AST contains:
    ...
    @classmethod
    def get_token(cls, ast: AST):
        return ast.token
    
    @classmethod
    def contains(cls, line: AST_LIST, keys: tuple):
        line = tuple(map(Scope.get_token, line))
        return any(key in line for key in keys)
    
    @classmethod
    def get_match(cls, line: AST_LIST, keys: tuple = None, depth: int = 0):
        all_keywords = tuple(KEYWORDS.keys()) if not keys else keys
    ...
    def process_scope_from_scope(cls, scope: 'Scope') -> 'Scope':
        ...
        for index in range(len(scope.children)):
            if not in_scope and cls.contains(scope.children[index], NAMESPACED_KEYWORD.keys()) and temp_scope.indent_level != scope.children[index].indent_level:
                new_scope = Scope(scope.children[index], NAMESPACED_KEYWORD[cls.get_match(scope.children[index], NAMESPACED_KEYWORD.keys())], [scope.children[index]], scope.children[index].indent_level)
            ...
            if in_scope:
                if scope.children[index].indent_level == new_scope.indent_level:
                    ...
                    new_scope = None
                    in_scope = False
                    if cls.contains(scope.children[index], NAMESPACED_KEYWORD.keys()):
                        new_scope = Scope(scope.children[index], NAMESPACED_KEYWORD[cls.get_match(scope.children[index], NAMESPACED_KEYWORD.keys())], [scope.children[index]], scope.children[index].indent_level)
                        new_scope.namespace_header = scope.children[index]
                        in_scope = True
                        continue
    ...
    @classmethod
    def process_from_lines(cls, lines: tuple[AST_LIST]) -> 'Scope':
        root_scope = Scope(lines[0].file, "root", [], 0)
        new_scope = None
        in_scope = False

        for index in range(len(lines)):
            if not in_scope and cls.contains(lines[index], NAMESPACED_KEYWORD.keys()):
                new_scope = Scope(lines[index], NAMESPACED_KEYWORD[cls.get_match(lines[index], NAMESPACED_KEYWORD.keys())], [lines[index]], lines[index].indent_level)
            ...
            if in_scope:
                if lines[index].indent_level == new_scope.indent_level:
                    ...
                    new_scope = None
                    in_scope = False
                    if cls.contains(lines[index], NAMESPACED_KEYWORD.keys()):
                        new_scope = Scope(lines[index], NAMESPACED_KEYWORD[cls.get_match(lines[index], NAMESPACED_KEYWORD.keys())], [lines[index]], lines[index].indent_level)
                        new_scope.namespace_header = lines[index]
                        in_scope = True
                        continue
    ...
    def get_keyword(self, internal_name: str) -> str:
        return next((keyword for keyword in KEYWORDS if KEYWORDS[keyword]["internal_name"] == internal_name), None)