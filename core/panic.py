...
def standalone_tokenize_line(line: str, preserve_spaces: bool = False) -> list[str]:
    ...
    pattern = re.compile(rf"""
        ...
        ({back_slash.join(r"\~\~.*".split())}[^\n]*)                                                                | # Single line comments (~~)
        ({back_slash.join(r"\~\*\~".split())}[\s\S]*?{back_slash.join(r"\~\*\~".split())})                          | # Multi line comments (~*~ ... ~*~)
        (\b\d+\.\d+\b)                                                                                              | # Decimal numbers
        (\b\w+\b)                                                                                                   | # Words (identifiers, keywords)
        ({'|'.join(['\\=\\=\\=', '\\!\\=\\=', '\\.\\.\\.', '\\=\\=', '\\!\\=', '\\-\\>', '\\<\\-', '\\<\\=', '\\>\\=', '\\&\\&', '\\-\\-', '\\:\\:', '\\|\\|', '\\+\\+', '\\+\\=', '\\-\\=', '\\*\\=', '\\/\\=', '\\&\\=', '\\|\\=', '\\^\\=', '\\%\\=', '\\*\\*', '\\<\\<', '\\>\\>', '\\<\\<\\=', '\\>\\>\\=', '\\=\\>', '\\@\\=', '\\_\\_', '\\?\\?', '\\?\\:', '\\?\\='])})                                      | # Double character operators
...
def panic(__error: Exception,
    ...
    if not lang: lines = [highlight_code(line) for line in lines[:-1]] + [lines[-1]]
    else: lines = [highlight_code(line, lang_dict[lang]) for line in lines[:-1]] + [lines[-1]]
    ...
    def mark_all(line: str, mark_line: str, mark_start: Optional[int] = None) -> str:
        ...
        if not tokenized_line[skip].isspace():
            tokenized_line = standalone_tokenize_line(''.join(tokenized_line[:skip] + [" "] + tokenized_line[skip:]), preserve_spaces=True)
        ...
        if follow_marked_order:
            ...
            for index, token in enumerate(tokenized_line):
                token = s_u2(token)
                if color_escape_pattern.match(token): continue
                total_char_so_far += len(token) if index >= skip else 0
                if index < skip: continue
                
                if mark_start and (total_char_so_far-len(token))+1 <= (mark_start):
                    if (mark_index < len(mark) and token == mark[mark_index]) and mark[mark_index] not in tokenized_line[index+1:index+5]:
                        mark_start -= 1
                        tokenized_line[index] = f"{secondary_error_color}{token}{reset}"
                        mark_line  += f"{secondary_error_color}{'~'*len(token)}{reset}"
                        mark_index += 1
                    else:
                        tokenized_line[index] = highlight_code(token, lang_dict[lang] if lang else "helix").strip() ####
                        mark_line += ' ' * len(token)
                    continue

                if mark_index < len(mark) and token == mark[mark_index] and index < line_end:
                    tokenized_line[index] = f"{primary_error_color}{token}{reset}"
                    mark_line  += f"{secondary_error_color}{'^'*len(token)}{reset}"
                    mark_index += 1
                elif mark_index < len(mark) and token.isspace() and index < line_end and mark_index > 0:
                    mark_line += f"{secondary_error_color}{'^'*len(token)}{reset}"
                else:
                    tokenized_line[index] = highlight_code(token, lang_dict[lang] if lang else "helix").strip() ####
                    mark_line += ' ' * len(token)
    ...
    def process_lines(line: str, index: int) -> str:
        ...
        line = line + (" " * (terminal_width - (len(s_u2(line))+1))) + f"{border_color}{chars['straight']}{reset}"
        return line
    ...
    if not no_lines:
        ...
        print(mark_all(mid_section[-1], f"{border_color}│ {gray}{'~'*len(str(line_no).center(len(str(line_no+(lines_to_print-1)))))}{reset}", mark_start=mark_start))
        print(f"{border_color}{chars['straight']}{' '*(terminal_width-2)}{chars['straight']}{reset}")
    ...
    if hex_code:
        ...
        somewhat_middle = f"{border_color}{chars['straight']} {chars['dash']*(terminal_width-4)} {chars['straight']}{reset}" if not no_lines else ""
    
    if (not multi_frame or (multi_frame and pos == 2)) and not no_lines: print(somewhat_middle)
    ...
    def process_message(message: str) -> list[str]:
        ...
        for line in message.split("\n"):
            for line2 in textwrap.wrap(line, terminal_width-4):
                output.append(f"{border_color}{chars['straight']}{reset} {line2.ljust(terminal_width-4)} {border_color}{chars['straight']}{reset}")
        if hex_code:
            return [out.replace(f"\'helix doc {_hex_code.group(1)}.{_hex_code.group(2)}\'", f"{second_color}\'helix doc {_hex_code.group(1)}.{_hex_code.group(2)}\'{reset}") for out in output]
        return output
    ...
    final_line = final_line.split(file)[0] + green + file + reset + gray + ":" + green + str(line_no) + border_color + final_line.split(":")[(1 if ":\\" != ":" + final_line.split(":")[1][0] else 2)][len(str(line_no)):] + reset
    
    # check if terminal width is even
    if not multi_frame or (multi_frame and pos == 2):
        print(final_line)
    else:
        print(f"{border_color}{chars['straight']} {final_line[6:-11]} {chars['straight']}{reset}")
        print("\n".join(process_message(message))) if pos == 2 else None
        
    
    lock.release()
    if not no_exit: exit(1) if not multi_frame or (multi_frame and pos == 2) else None
