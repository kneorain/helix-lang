...
def _process_bg_color(bg: str) -> str:
    ...
    if not bg.startswith('rgb('):
        ...
        bg = bg.split('rgb')[1].split('(')[1].split(')')[0].split(',')
        if len(bg) != 3 or not isdigit(bg) or any([int(i.strip()) > 255 for i in bg]): 
            raise ValueError('Invalid rgb color ' + str(bg))
        for index, i in enumerate(bg): bg[index] = int(i.strip())
        bg = _make_bg_color(bg[0], bg[1], bg[2])
    return bg
