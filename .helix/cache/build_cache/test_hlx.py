...
# --------------------------------------------------------------------------------
# Filename: test.hlx
# Generation Date: 2024-03-15 00:33:53
...
def exception_handler(exception_type: type[BaseException] | threading.ExceptHookArgs, exception: Optional[Exception] = None, tb: Optional[types.TracebackType] = None, debug_hook: bool = False, thread_error: bool = False):
    ...
    relevant_frames = []
    early_replacements = dict((v, k) for k, v in {'...': 'None', 'true': 'True', 'false': 'False', 'null': 'None', 'none': 'None', 'await': '_await', '&&': 'and', '||': 'or', '!': 'not', '===': 'is', '!==': 'is not', 'stop': 'break', '::': '.', 'new': '__init__', 'delete': '__del__', 'enter': '__enter__', 'exit': '__exit__', 'u8': 'hx_u8', 'u16': 'hx_u16', 'u32': 'hx_u32', 'u64': 'hx_u64', 'u128': 'hx_u128', 'i8': 'hx_i8', 'i16': 'hx_i16', 'i32': 'hx_i32', 'i64': 'hx_i64', 'i128': 'hx_i128', 'f32': 'hx_f32', 'f64': 'hx_f64', 'f128': 'hx_f128'}.items())
...
def main(argv: list[str | hx_string] | hx_list[str | hx_string]):
    ...
    b: int | hx_int = (int())
    print ( a , b )
    for i, j in C_For(i = int(0), j = int(0)).set_con('i < 10').set_inc('i ++ ; j += 3'):
        printf ( "i: %d, j: %d" , i , j )
    return 0
@hx__async
...
def subtract(a: int | hx_int, b: int | hx_int) -> int:
    some_map: dict[str | hx_string , int | hx_int] | hx_map[str | hx_string , int | hx_int] = dict({ "a" : 1 , "b" : 2 , "c" : 3 })
    some_list: list[int | hx_int] | hx_list[int | hx_int] = list([ 1 , 2 , 3 ])
    some_set: set[int | hx_int] | hx_set[int | hx_int] = set({ 1 , 2 , 3 })
    some_tuple: tuple[int | hx_int] | hx_tuple[int | hx_int] = tuple(( 1 , 2 , 3 ))
    some_string: str | hx_string = str("hello")
    some_char: str | hx_char = str('a')
    some_bool: bool | hx_bool = bool(True)
    some_float: float | hx_float = float(1.0 / 3.0)
    some_double: double | hx_double = double(1.0 / 3.0)
    some_int: int | hx_int = int(2 ** 9046)
    print ( "map:" , some_map )
