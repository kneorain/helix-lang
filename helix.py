...
import multiprocessing, black
def multi_exec(code: str, *args, **kwargs):
    return multiprocessing.Process(target=exec, args=(code, *args), kwargs=kwargs, daemon=True).start()
...
def main():
    ...
    a = tokenize_file("syntax/test.hlx")
    b = Scope.process_from_lines(a)
    c = Transpiler.transpile(b)
    code = '\n'.join([c.to_code() for c in c])
    code = "# This file was automatically generated by the Helix transpiler\n" + \
    ...
    subprocess.run([sys.executable, "output.py"])
